...
export function valid_line(numbers: number[], tolerance: number = 0): boolean {
	...
	console.log(`amount of errors for "${numbers}" is ${errors} with tolerance ${tolerance}`);
	return errors <= tolerance;
}

export async function save_reports(filename: string, tolerance: number = 0): Promise<number> {
